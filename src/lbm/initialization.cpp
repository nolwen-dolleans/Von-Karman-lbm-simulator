...
void setup_init_state_global_poiseuille_profile(Mesh* mesh, lbm_mesh_type_t* mesh_type, const lbm_comm_t* mesh_comm) {
  ...
  for (size_t j = 0; j < mesh->height; j++) {
    ...
    for (size_t k = 0; k < DIRECTIONS; k++) {
      // Compute equilibrium
      lbm_mesh_cell_t cell0 = Mesh_get_cell(mesh, 0, j);
      lbm_mesh_cell_t cell1 = Mesh_get_cell(mesh, 1, j);
      cell0[k]              = compute_equilibrium_profile(v, rho, k, v2);
      cell1[k]              = compute_equilibrium_profile(v, rho, k, v2);
      // Mark as standard fluid
      *(lbm_cell_type_t_get_cell(mesh_type, 0, j)) = CELL_FUILD;
      *(lbm_cell_type_t_get_cell(mesh_type, 1, j)) = CELL_FUILD;
    }

    for (size_t i = 2; i < mesh->width; i++) {
      for (size_t k = 0; k < DIRECTIONS; k++) {
        Mesh_get_cell(mesh, i, j)[k] = equil_weight[k];
      }
    }
  ...
  if (mesh_comm->top_id == MPI_PROC_NULL) {
    for (size_t i = 0; i < mesh->width; i++) {
      for (size_t k = 0; k < DIRECTIONS; k++) {
        // Compute equilibrium.
		    double v2 = get_vect_norm_2(v, v);
        Mesh_get_cell(mesh, i, 0)[k] = compute_equilibrium_profile(v, rho, k, v2);
        // Mark as bounce back
  ...
  if (mesh_comm->bottom_id == MPI_PROC_NULL) {
    for (size_t i = 0; i < mesh->width; i++) {
      for (size_t k = 0; k < DIRECTIONS; k++) {
        // Compute equilibrium.
		double v2 = get_vect_norm_2(v, v);
        Mesh_get_cell(mesh, i, mesh->height - 1)[k] = compute_equilibrium_profile(v, rho, k, v2);
        // Mark as bounce back
