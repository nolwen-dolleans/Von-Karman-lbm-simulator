...
void init_cond_velocity_0_density_1(Mesh* mesh) {
  ...
  for (size_t i = 0; i < mesh->width; i++) {
    for (size_t j = 0; j < mesh->height; j++) {
		auto cell = Mesh_get_cell(mesh, i, j);
  ...
  for (size_t i = 0; i < mesh->width; i++) {
    for (size_t j = 0; j < mesh->height; j++) {
      ...
      for (size_t k = 0; k < DIRECTIONS; k++) {
        ...
        cell[k]              = compute_equilibrium_profile(v, rho, k, 0);
...
void setup_init_state_border(Mesh* mesh, lbm_mesh_type_t* mesh_type, const lbm_comm_t* mesh_comm) {
  ...
  if (mesh_comm->top_id == -1) {
    for (size_t i = 0; i < mesh->width; i++) {
      for (size_t k = 0; k < DIRECTIONS; k++) {
        // Compute equilibrium.
        Mesh_get_cell(mesh, i, 0)[k] = compute_equilibrium_profile(v, rho, k, 0);
        // Mark as bounce back
  ...
  if (mesh_comm->bottom_id == -1) {
    for (size_t i = 0; i < mesh->width; i++) {
      for (size_t k = 0; k < DIRECTIONS; k++) {
        // Compute equilibrium.
        Mesh_get_cell(mesh, i, mesh->height - 1)[k] = compute_equilibrium_profile(v, rho, k, 0);
        // Mark as bounce back
